...
export async function uploadVersionJSON({
  ...
  console.log('Asset names: ', Array.from(assetNames).join(','));

  const filteredAssets = assets.data.filter((data) =>
    assetNames.has(data.name),
  );

  console.log('Filtered assets: ', filteredAssets.map(a => a.name).join(','));

  console.log('Signature file: ', signatureFile.path)

  const baseName = basename(signatureFile.path, extname(signatureFile.path));

  console.log('Signature file baseName: ', baseName)
